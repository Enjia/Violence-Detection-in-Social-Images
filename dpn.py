...
#Function of adaptive_avgmax_pool2d
...
model_urls = {
    ...
    'dpn107-extra':
        'http://data.lip6.fr/cadene/pretrainedmodels/checkpoint_DPN.pth.tar',
}
...
def dpn107(num_classes=1000, pretrained=False, test_time_pool=True):
    ...
    if pretrained:
        if model_urls['dpn107-extra']:
            model.load_state_dict(model_zoo.load_url(model_urls['dpn107-extra'], model_dir='/search/odin/Enjia/DPN/'))
        elif has_mxnet and os.path.exists('./pretrained/'):
...
class FinalLayer(nn.Module):
    ...
    def __init__(self):
        ...
        self.classifier = nn.Conv2d(2688, 12, kernel_size=1, bias=True)
...
def dpn107(num_classes=1000, pretrained=False, test_time_pool=True):
    model = DPN(
        ...
        k_sec=(4, 8, 20, 3), inc_sec=(20, 64, 64, 128),
        num_classes=num_classes, test_time_pool=test_time_pool)
    model.classifier = FinalLayer()
    if pretrained:
        if model_urls['dpn107-extra']:
            with open('/search/odin/Enjia/ViolenceDetectionV2/checkpoint_DPN.pth.tar', 'rb') as f:
